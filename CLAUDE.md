...
2. **templates/ や design-system/ を変更する場合は HITL フラグを立てる**（featureブランチ + PR必須）
...
ビルドログに CSS フィンガープリントが表示されます:
```
✓ dist/assets/style.css  (v=3a7f1c2b)
```
CSS が変更されるたびにハッシュが変わり、ブラウザキャッシュが自動的に無効化されます。
...
| `🚫 HITL violation` | templates/ を直接 main に push した | feature/* ブランチを作って PR 経由で作業する |

---

## GitHub MCP 接続のセットアップ

初回セットアップや接続が切れた場合は [docs/SETUP_GITHUB_MCP.md](docs/SETUP_GITHUB_MCP.md) を参照。

GitHub App の作成・権限設定・リポジトリへのインストール・
Claude.ai 側の Remote MCP Server 設定（Client ID / Client Secret）まで
すべての手順を記載しています。
